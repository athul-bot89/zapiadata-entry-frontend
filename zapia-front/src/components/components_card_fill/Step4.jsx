...
const Step4 = ({ onNext, onPrev, formData, setFormData }) => {
  ...
  const handleSubmit = async () => {
    // Check if at least root node is filled
    if (!selectedNodes.has('root') || !nodeData['root']?.no_of_points) {
  ...
  return (
    <div className="space-y-6 min-h-[600px] pb-6">
      ...
      {/* Points Category Tree Selection */}
        ...
        <div className="border-b border-gray-100 p-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-800">Category Configuration</h3>
                <p className="text-sm text-gray-500">Select and configure reward categories</p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div className="text-sm">
                ...
                <ul className="text-amber-800 space-y-1">
                  <li>• The <span className="font-semibold bg-purple-100 text-purple-700 px-2 py-0.5 rounded">root node</span> is mandatory and sets default points</li>
                  <li>• Select specific categories for custom point rates</li>
                  <li>• Use checkboxes to select, arrows to expand/collapse</li>
                </ul>
              </div>
            </div>
          </div>
      ...
      {/* Navigation buttons */}
      <div className="mt-8">
        {/* Show warning if root node is not configured */}
        {(!selectedNodes.has('root') || !nodeData['root']?.no_of_points) && (
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-red-700">
                  ...
                  The root node defines the default points for all transactions.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-4">
            {selectedNodes.size > 0 && (
              <div className="text-sm text-gray-500">
                <span className="font-semibold text-gray-700">{selectedNodes.size}</span> categories configured
              </div>
            )}
            
            <button
              onClick={handleSubmit}
              disabled={loading || !selectedNodes.has('root') || !nodeData['root']?.no_of_points}
              className={`flex items-center px-6 py-3 rounded-lg font-medium transition-all duration-200 transform hover:scale-105 ${
                loading || !selectedNodes.has('root') || !nodeData['root']?.no_of_points
                  ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                  : 'bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:from-blue-700 hover:to-purple-700 shadow-lg'
              }`}
              title={!selectedNodes.has('root') || !nodeData['root']?.no_of_points 
